...
4. **5 个 expert 章节的 TL;DR 速览**（每个章节 ≤30 字一句话摘要）

专家段落会由系统端**自动嵌入**到最终 markdown，你不需要写正文；但你**必须**根据 `{{experts_summary}}` 提炼出 5 个章节各自的 TL;DR，让读者扫一眼就能决定要不要深读。
...
```json
{
  ...
  "bucket_takeaways": {
    "launch": "≤30 字一句话章节摘要：变现缝隙章节本期最锋利的判断（谁能赚钱 / 谁的钱被谁抢了）",
    "tech": "≤30 字一句话章节摘要：底层基建章节本期最值得关注的技术风向（哪个模型/工具值得本周测）",
    "competition": "≤30 字一句话章节摘要：逆向拆解章节本期的核心竞争洞察（谁在蚕食谁）",
    "demand": "≤30 字一句话章节摘要：痛点狙击章节本期最值得跟进的需求（开发者最痛的痒点）",
    "trend": "≤30 字一句话章节摘要：过滤噪音章节本期的趋势判断（哪个看似机会其实是陷阱 / 反之）"
  },
  "__bucket_takeaways_note": "每条 ≤30 个中文字符，基于 `{{experts_summary}}` 对应章节的内容提炼。要求：① 第一人称锋利动词（'蚕食'、'抢走'、'吃掉'、'必须'）；② 不写元叙述（禁止'本章讨论 xxx'）；③ 不带 emoji；④ 结尾不加句号；⑤ 不与 top_signals 文案重复；⑥ 用于 Web 详情页 H2 章节摘要卡 + 首屏速览。输出时不要保留 __bucket_takeaways_note 这个说明键。"
}
```
